/*
 *
 * Code wrote by: Br. David Carlson (1989), found at:
 * http://www.mygnet.net/foros/miscelanea/juego-del-gato-en-prolog.2894
 *
 *
 * Code explanation and other key points commented in Spanish by: David Osorio & Williams Estrada
 * on June 04, 2018
 *
 * Original comments remain in English
 *
*/
...
/* Ingresar go. para empezar a jugar, cargar instrucciones y preguntar al usuario parámetros del juego */
go :-
    nl,
    write('Playing tic-tac-toe...'),
    nl,
    explainMoves, !,
    games, !,
    nl,
...
explainMoves :-
    nl, write('You choose a move by giving the number of the'),
    nl, write('square to which you wish to move:'), nl, nl,
    write(' 1 | 2 | 3'), nl, printline,
    write(' 4 | 5 | 6'), nl, printline,
    write(' 7 | 8 | 9'), nl.

/* Imprime una línea ---- para separar las filas del tablero */
printline :-
    write(' -----------'),
    nl.

/* Pregunta al usuario si quiere ser X ó O y quién empieza primero. Una vez acabado el gato, pregunta si
quiere jugar otro. Si la respuesta es 'y' (yes) se llama a sí mismo recursivamente mediante process(Response) */
games :-
    game, !,
    optionalGames.

optionalGames :-
    nl, write('Care for another game (y/n)? '),
    inputResponse(Response), !,
    process(Response).

game :-
    nl, write('Do you wish to be x or o? '),
    inputPlayer(Player),
    nl, write('Do you want to go first (y/n)? '),
    inputResponse(PlayerMoves), !,
    play(Player, PlayerMoves, 9, board(_,_,_,_,_,_,_,_,_)).

/* Determina si el usuario es 'x' ó 'o' */
inputPlayer(Player) :-
    read(Player),
    x_or_o(Player). /* error check */

inputPlayer(Player) :-
    write('Not a valid response -- type x or o: '),
    inputPlayer(Player). /* reinput Player */

/* Las únicas respuestas válidas para el tipo de jugador son 'x' ó 'o' */
x_or_o(x).
x_or_o(o).

/* Determina si el jugadar quiere o no empezar primero */
inputResponse(Response) :-
    read(Answer),
    okResponse(Answer, Response), !.

/* Error check and put in standard form */
inputResponse(Response) :-
    write('Not a valid response -- type y or n: '),
    inputResponse(Response). /* reinput */

/* Las diferentes respuestas válidas ingresadas son transformadas a 'yes' o 'no' */
...
/* Imprime el tablero con las 9 celdas. writeVal() escribe el valor que tiene, puede ser ' ', 'x' ó 'o' */
...
oneMove(Player, yes, Board) :-
    !, inputMove(Board, Location),
    makeMove(m(Location, Player), Board). /* human player moves */

oneMove(Player, no, Board) :-
...
/* First, this checks if computer can win with this move */
generateMove(Computer, Board, Location) :-
    slice(Pos1, Pos2, Pos3),
    argb(Pos1, Board, Val1),
    argb(Pos2, Board, Val2),
    argb(Pos3, Board, Val3),
    winningMove(Computer, Pos1, Pos2, Pos3, Val1, Val2, Val3, Location), !.

/* Then, this checks if computer is forced to move */
generateMove(_, Board, Location) :-
    slice(Pos1, Pos2, Pos3),
    argb(Pos1, Board, Val1),
    argb(Pos2, Board, Val2),
    argb(Pos3, Board, Val3),
...
*/
generateMove(Computer, Board, Pos3) :-
    diagonal(Pos1, Pos2, Pos3),
    argb(Pos1, Board, Val1),
    argb(Pos2, Board, Val2),
    argb(Pos3, Board, Val3),
...
generateMove(Computer, Board, Pos3) :-
    ...
    argb(Pos3, Board, Val3),
    neighbors(Pos3, NbrA, NbrB),
    argb(NbrA, Board, ValA),
    argb(NbrB, Board, ValB),
...
generateMove(_, Board, 5) :-
    argb(5, Board, Val), var(Val), !.

/* Computer prefers a corner position */
generateMove(_, Board, Location) :-
    openCorner(Board, Location), !.

/* Computer takes any open square */
generateMove(_, Board, Location) :-
    location(Location),
    argb(Location, Board, Val), var(Val), !.

/* 8 movimientos ganadores posibles sin importar sentido */
slice(1,2,3).
slice(4,5,6).
slice(7,8,9).
slice(1,4,7).
...
/* Diagonales ganadoras en ambos sentidos */
diagonal(1,5,9).
diagonal(9,5,1).
diagonal(3,5,7).
diagonal(7,5,3).
...
winningMove(Computer, Pos1,_,_, Val1, Val2, Val3, Pos1) :-
    var(Val1),
    nonvar(Val2), Val2 == Computer, nonvar(Val3), Val3 == Computer, !.

winningMove(Computer,_, Pos2,_, Val1, Val2, Val3, Pos2) :-
    var(Val2),
    nonvar(Val1), Val1 == Computer, nonvar(Val3), Val3 == Computer, !.

winningMove(Computer,_,_, Pos3, Val1, Val2, Val3, Pos3) :-
    var(Val3),
    nonvar(Val1), Val1 == Computer, nonvar(Val2), Val2 == Computer, !.

/* Computer is forced to move if Player has 2 in a row, 3rd square open */
forcedMove(_,_, Pos3, Val1, Val2, Val3, Pos3) :-
    var(Val3),
    nonvar(Val1), nonvar(Val2), Val1 == Val2, !.

forcedMove(_, Pos2,_, Val1, Val2, Val3, Pos2) :-
    var(Val2),
    nonvar(Val1), nonvar(Val3), Val1 == Val3, !.

forcedMove(Pos1,_,_, Val1, Val2, Val3, Pos1) :-
    var(Val1),
    nonvar(Val2), nonvar(Val3), Val2 == Val3, !.

/* This special move was added 11/13/93 to block a known way to beat the program.
The suggested move is to Pos3 */
specialMove(Computer, Val1, Val2, Val3, ValA, ValB) :-
...
/* This special move was also added 11/13/93 to block a known way to beat the program.
The suggested move is toPos3 */
special2Move(Computer, Val1, Val2, Val3, ValA, ValB) :-
    var(Val2), var(Val3), nonvar(Val1), Val1 == Computer,
...
goodMove(Computer,_, Pos3, Val1, Val2, Val3, Pos3) :-
    var(Val2),
    var(Val3), nonvar(Val1), Val1 == Computer, !.

goodMove(Computer, Pos1,_, Val1, Val2, Val3, Pos1) :-
    var(Val1),
...
continuePlay(Player,_,_, Board) :-
    won(Who, Board), /* check for win */
...
/* Imprime mensaje si el usuario gana */
winnerOut(Player, Player) :-
    !, write('You won. Congratulations!'), nl.

/* Imprime mensaje si la computadora gana */
winnerOut(_,_) :-
    write('I won!'), nl.
...
couldBeWin(Mover, [Pos], Board) :-
    test1(Mover, Pos, Board).

/* Test if move to Pos1 and next move to Pos2 leads to winner */
test2(Mover, Pos1, Pos2, Board) :-
    argb(Pos1, Board, Mover),
    opposite(Mover, Next),
    argb(Pos2, Board, Next),
    win(Board).

/* Test if move to Pos leads to winner */
test1(Mover, Pos, Board) :-
    argb(Pos, Board, Mover),
    win(Board).
